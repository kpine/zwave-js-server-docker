...
build:
  ...
  LET ZWAVE_JS_PACKAGE=zwave-js@$ZWAVE_JS_VERSION
  LET ZWAVE_JS_SERVER_PACKAGE=@zwave-js/server@$ZWAVE_JS_SERVER_VERSION
  LET ZWAVE_JS_FLASH_PACKAGE=@zwave-js/flash@$ZWAVE_JS_VERSION
...
docker:
  ...
  LET VERSION="$ZWAVE_JS_SERVER_VERSION-$ZWAVE_JS_VERSION"
  LET BUILD_DATE=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
...
docker-release:
  ...
  ARG REGISTRY=docker.io
  ARG REPOSITORY=kpine/zwave-js-server
  LET TAG="$ZWAVE_JS_SERVER_VERSION-$ZWAVE_JS_VERSION"
