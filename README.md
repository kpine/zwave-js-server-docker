...
- `LOGFILENAME`: Set this to [configure](https://zwave-js.github.io/node-zwave-js/#/api/driver?id=logconfig) the driver log filename (only used when `LOGTOFILE` is `true`). The default is `/logs/zwavejs_`, which results in files named `zwavejs_%DATE%.log`. Note that the driver will automatically rotate the log files using the date based scheme.
